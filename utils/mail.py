"""
邮件类。用来给指定用户发送邮件。可指定多个收件人(分号隔开)，可带附件。
"""
__author__ = 'zhaicao'
...
class Email():
    ...
    def __init__(self, receiver, title, message=None, path=None, acc=None, server=conf.get('smtpserver'), sender=conf.get('sender'), password=conf.get('password')):
        """初始化Email
        :param title: 邮件标题，必填。
    ...
    def send(self):
        ...
        self.msg['Cc'] = self.acc
        # 附上抄送人员
        self.receiver += ';' + self.acc if self.acc else ''
